...
**หลัง `git pull`:** รัน `zsh tools/deploy.sh` อีกครั้งเพื่อ refresh path ของ hooks — ปลอดภัยแม้มีแค่แพลตฟอร์มเดียว
...
เมื่อ detect Hermes ได้ `deploy.sh` ติดตั้ง team skills (`loop-start loop-orch pm design fe fe-anim be be-sr qa LOOP`)
...
> ทั้งสามแพลตฟอร์มอ่าน `loop.config.json` จาก **โฟลเดอร์ที่คุณอยู่** เริ่มด้วย `loop-start` เพื่อปักโปรเจกต์ที่ถูก **ไม่จำเป็นต้องมีครบทั้งสาม** — เลือกอย่างใดอย่างหนึ่งก็พอ
