...
> ```
>
> แล้ว **restart Claude Code และ Cursor** ให้โหลด hooks ใหม่
>
> `git pull` อย่างเดียว **ไม่** อัปเดต hooks — หลังย้ายโฟลเดอร์ให้รัน `deploy.sh` (หรืออย่างน้อย `zsh tools/install-dash-hooks.sh` และเขียน `~/.loop-base` ใหม่)
>
...
**ส่ง activity ไป dashboard อัตโนมัติ (Claude Code + Cursor hooks):**

```zsh
zsh tools/install-dash-hooks.sh   # รวมใน deploy.sh แล้ว
```

หลังติดตั้ง → **restart Claude Code และ/หรือ Cursor** — hooks สะท้อน:
- แก้/สร้างไฟล์ (`Edit` / `Write` / `StrReplace` / `Delete`)
- คำสั่ง shell ที่มี label อ่านง่ายเท่านั้น (เช่น `npm test`, `eslint` — ไม่ dump raw debug)
- สรุป sub-agent / จบ session (เมื่อข้อความยาวพอ)

ชื่อโปรเจกต์ resolve จาก `loop.config.json` ใน cwd, walk ขึ้นหาโฟลเดอร์แม่, หรือ `.active-project` ใน Loom — ไม่ tag `(unknown)`

Hermes และแชต manual ยังได้ประโยชน์จาก `dash.sh report` / `say` โดยตรง
...
**Auto-bridge:** ใช้ `install-dash-hooks.sh` ร่วมกับ Claude Code (ดูด้านบน) — Cursor ได้ hooks เพิ่ม (`afterFileEdit`, `afterShellExecution`, `afterAgentResponse`)
