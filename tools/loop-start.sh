#!/usr/bin/env zsh
# Bootstrap base folder + control folder — same steps as loop-start / /loop-start skill.
#   zsh tools/loop-start.sh                         full wizard (Steps 1–4)
#   zsh tools/loop-start.sh --new NAME [--base DIR] Step 1 + 2b + 3 + 4 (new-project compat)
#   zsh tools/loop-start.sh --open PATH             Step 3 + 4 only (resume existing control)
set -euo pipefail
