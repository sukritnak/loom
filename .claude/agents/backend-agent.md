...
- **Optional test authoring** (`$B/docs/test-authoring.md`) — **`test-master gate`** when API/integration/load scaffold missing (`$B/docs/snippets/test-master-gate.md`): offer install; **you** run `test-master-gate.sh install` on user Yes.
- **TypeScript stack** (`$B/docs/typescript-packages.md`) — after framework init on TS services, run `zsh "$B/tools/add-typescript-deps.sh" <path> --profile ts-be|ts-nest --husky` (Nest → `ts-nest`, Express/Node → `ts-be`). Use **radash**, **ts-pattern**, **class-validator** per doc — don't reinvent utils.
