...
| `loom-qa` | QA | AC → PASS/FAIL · FE/UI ผ่าน **`qa-browser`** (checker — แยกจาก makers) |
...
| แก้ API อย่างเดียว | PM → BE → SR → QA (ข้าม UX/FE) |
...
| ตรวจ | `loom-qa` | tests + `qa-browser` — ไม่เชื่อ self-report ของ maker |
...
**`qa-browser`** รวมใน `zsh tools/init.sh` — ดู [ขั้น 1](#1-ติดตั้งทีม-ครั้งเดียวต่อเครื่อง--รันจาก-base)

สเปก loop → [LOOP.md](LOOP.md) · Process gates → [docs/loop-process.md](docs/loop-process.md) · สถาปัตยกรรม → [docs/hexagonal-project-structure.md](docs/hexagonal-project-structure.md)
...
tools/                     only in Base — every control folder shares via ~/.loop-base
  init.sh                install team to Claude Code + Hermes + register Base at ~/.loop-base
