...
> **เริ่มต้น:** [README.md](README.md) · [README-TH.md](README-TH.md)
>
> **เอกสารเต็ม:** [English](README.full.md) · **ไทย** (เอกสารนี้)
>
...
**อัปเดตล่าสุด (v1.0.8):** Browser QA (local CDP + cloud), แยก verify FE/BE, test-master แบบถามก่อนลง, ชุด TypeScript — ดู [ด้านล่าง](#whats-new-v108)

**ก่อนหน้า (2026-07-01):** เลือก platform + model ต่อ agent — ดู [What's new](#whats-new-2026-07-01)
...
QA, การตรวจ และเครื่องมือ TypeScript — **pull แล้ว refresh** เพื่ออัปเดตเครื่อง
...
`refresh.sh` รันอัตโนมัติผ่าน **git hooks** ตอน merge/checkout ข้าม browser stack ได้ด้วย `INIT_SKIP_EXTERNAL_SKILLS=1`

### Browser QA — สอง backend (FE/UI เท่านั้น)

| `qa_browser` | Backend | ต้องมี API key? |
|--------------|---------|-----------------|
| **`local-cdp`** *(default ผ่าน `auto`)* | [chrome-devtools-mcp](https://github.com/ChromeDevTools/chrome-devtools-mcp) หรือ Cursor `cursor-ide-browser` | ไม่ |
| **`browser-use`** | [browser-use](https://github.com/browser-use/browser-use) + skill `qa-browser` | ใช่ — orch gate A/B/C |
| **`auto`** | local-cdp ถ้ามี MCP ไม่งั้น browser-use | ขึ้นกับ mode |

- **BE / API AC** → `npm test`, curl — **ไม่ใช้ browser** ไม่ gate ไม่ต้อง `BROWSER_USE_API_KEY`
- **UI AC** → browser จริง PASS/FAIL ต่อข้อ
- เก็บ key ที่ `~/.loom/browser-use.env` (gitignore) — ดู [docs/browser-qa.md](docs/browser-qa.md)

`loom-start` / `init-config.sh` ถาม Browser QA · `loop.config.json` → `qa_browser`

### เส้นทาง verify (FE vs BE)

| ประเภท AC | เครื่องมือ |
|-----------|-----------|
| UI / layout / flow | `local-cdp` หรือ `qa-browser` |
| API / DB / jobs | เทสโปรเจกต์ + curl |
| FE performance (CWV) | `perf-lighthouse` บน **fe / fe-mo เท่านั้น** |
| BE load / SLO | k6 / สคริปต์โปรเจกต์ (ไม่ใช่ Lighthouse) |

ตารางเต็ม → [docs/test-authoring.md](docs/test-authoring.md)

### ทางเลือก: test-master (agent ลงให้หลังคุณกด OK)

เมื่อ loop ต้องการ scaffold เทส (integration, matrix, flaky mock, OWASP/load) agent ถาม **ใช่ / ไม่ใช่ตอนนี้** — **ไม่ต้อง copy คำสั่ง** กดใช่แล้ว agent รัน `test-master-gate.sh install` ไม่รวมใน `init.sh` default

### ชุด TypeScript มาตรฐาน

หลัง init Nest / Express / Vite / Next:
...
Profiles: **radash**, **ts-pattern**, **lodash**, **dayjs**, **class-validator**, **passport/jwt**, **jest/eslint/prettier**, **husky** pre-commit — ดู [docs/typescript-packages.md](docs/typescript-packages.md)

### Process gates (v1.0.7+)

ทีม 9 ตัวเดิม ลำดับเข้มขึ้น — [docs/loop-process.md](docs/loop-process.md): evidence · debug บั๊ก · SR 2 ขั้น · plan batches · finish checklist

### tools ใหม่ / อัปเดต

| Tool | หน้าที่ |
|------|---------|
| `install-browser-qa.sh` | MCP + qa-browser (รันตอน `refresh.sh`) |
| `qa-browser-gate.sh` | resolve mode + gate API key ก่อน UI QA |
| `test-master-gate.sh` | ลง test-master แบบ agent ถาม user |
| `add-typescript-deps.sh` | profiles npm TypeScript ของ Loom |
| `ensure-loom-home.sh` | โครง `~/.loom/` สำหรับ secrets |
| `ensure-control-gitignore.sh` | gitignore control folder สำหรับ key |

## Quick start — Claude Code

**Clone แล้วใช้ได้เลย** (ไม่ต้องรัน `init.sh` เอง — ครั้งแรกที่ `./loom` จะลงทะเบียนให้อัตโนมัติ):
...
| **ทีม agent** | `.claude/agents/` | — (ติดตั้งระดับเครื่องผ่าน `init.sh`) |
...
- tools + dashboard หา Base ผ่าน `~/.loop-base` (เขียนโดย `init.sh` หรือ `new-project.sh`)
...
> **Blueprint (Base = repo Loom นี้)** ไม่ได้สร้างโดย `loom-start` — clone repo แล้ว `./loom` ครั้งแรกจะ bootstrap ให้ (หรือ `zsh tools/init.sh` สำหรับติดตั้งเต็ม)
...
```mermaid
flowchart TD
  START([งานใหม่]) --> ORCH[loom-orch<br/>STATE + scope]

  ORCH --> KIND{task_kind}
  KIND -->|bug| DBG[Debug gate<br/>repro + Debug log]
  KIND -->|audit-only| FSREV[loom-full-stack<br/>รีวิวอย่างเดียว]
  KIND -->|feature| PM

  DBG --> PM[loom-pm<br/>acceptance criteria]
  PM --> UI{มี UI ใน scope?}
  UI -->|ใช่| UX[loom-ux-ui]
  UI -->|ไม่| BUILD
  UX --> BUILD[Build]
  ...
  TRI -->|รอบ ≤ 3| BUILD
  TRI -->|ติด| HUMAN([Human gate])
  FSREV --> DONE
```

> **Scope:** orch ส่งงานให้ maker **เฉพาะที่เกี่ยว** — ไม่ใช่ทุกกล่องทุกครั้ง ดู diagram ด้านล่าง
...
สเปก process: [`docs/loop-process.md`](docs/loop-process.md) · Handoff: [`docs/handoff.md`](docs/handoff.md)
...
| **refresh.sh** | ทุกครั้งที่ pull: agents · **chrome-devtools-mcp** · **qa-browser** (ยกเว้น `INIT_SKIP_EXTERNAL_SKILLS=1`) |
...
**แพลตฟอร์มเป็นทางเลือก — `init.sh` ไม่ fail ถ้าไม่มีตัวใดตัวหนึ่ง**
...
| แพลตฟอร์ม | detect เมื่อ | `init.sh` ติดตั้ง | ถ้าไม่มี |
| --------- | ------------ | ------------------- | -------- |
...
zsh tools/init.sh                       # หรือ: zsh tools/install-dash-hooks.sh
...
**External skills ที่ init ติดตั้ง**
...
| `perf-lighthouse` | fe, fe-mo | ตรวจประสิทธิภาพเว็บ (FE เท่านั้น — ไม่ใช่ BE) |
| `qa-browser` | qa | ทดสอบ FE/UI บนเบราว์เซอร์จริง |
| `test-master` *(optional)* | be, fe, qa, pm | เขียนเทสตามสถานการณ์ — agent ถามแล้วลงให้ผ่าน `test-master-gate.sh` |
...
> **⚠ ย้ายโฟลเดอร์ Loom หลัง `init.sh`**
>
...
```zsh
zsh tools/init.sh                  # ติดตั้งทีม (ครั้งเดียวต่อเครื่อง)
...
| `loom-full-stack` | Senior Fullstack | **SR code review** (2 ขั้น) ก่อน QA · bootstrap hex · DB/security maker |
| `loom-qa` | QA | AC → PASS/FAIL · **BE:** tests/curl · **FE/UI:** `local-cdp` หรือ `qa-browser` (checker) |
...
| คุณขอ… | Loom ทำ… |
|--------|----------|
| ฟีเจอร์ใหม่ (full stack) | PM AC → UX spec → BE + FE คู่ขนาน → SR review → QA browser + tests |
| แก้ API อย่างเดียว | PM → BE → SR → QA (tests/curl — **ไม่ใช้ browser**) |
| UI / motion | PM → UX → FE หรือ **fe-mo** (3D/WebGL) → SR → QA |
| แก้บั๊ก | Repro + debug log → แก้ → evidence → SR → QA regression |
| รีวิวสถาปัตยกรรม | **audit-only** — fullstack L1 ไม่ build ฟีเจอร์ |
| โค้ด legacy | Orientation ก่อน — สำรวจสไตล์, ponytail-review แล้วค่อยแก้ |
| โปรเจกต์ใหม่ | Scaffold BE แบบ hex + FE clean layers (`mode: new`) |

### ทีมทำงานร่วมกันอย่างไร

```mermaid
flowchart LR
  YOU([คุณ]) --> ORCH[loom-orch]
...
| ขั้น | Agent | หน้าที่ |
|------|-------|---------|
| วางแผน | `loom-pm`, `loom-ux-ui` | AC, flow, `## Plan` — ไม่เขียนโค้ด production |
| สร้าง | `loom-be`, `loom-fe`, `loom-motion` | implement + `Verified:` ใน handoff |
| รีวิว | `loom-full-stack` | SR Stage A (สัญญา) → B (ponytail) — **หลัง** makers คนละรอบ |
| ตรวจ | `loom-qa` | **BE:** `npm test` / curl · **UI:** agent browser (`local-cdp` / `qa-browser`) — ไม่เชื่อ self-report |

**แยกบทบาท (ไม่ทับกัน):**

| บทบาท | Agent | ห้าม… |
|--------|-------|-------|
| วางแผน | pm, ux-ui | เขียนโค้ด production |
| สร้าง | be, fe, fe-mo, fullstack (maker) | บอกว่า PASS เอง |
| รีวิว | fullstack (รอบ reviewer) | ทำหน้าที่ QA; ข้าม ponytail |
| ตรวจ | qa | implement ฟีเจอร์ |

**Process gates** ([`docs/loop-process.md`](docs/loop-process.md)) — ทีมเดิม 9 ตัว แต่ลำดับเข้มขึ้น:

1. **Evidence** — maker ใส่ `Verified:` (คำสั่ง + exit code) ใน handoff
2. **Debug** — บั๊กต้องมี repro + `## Debug log` ก่อนแก้
3. **SR review** — fullstack 2 ขั้น ก่อน QA
4. **Plan batches** — งานใหญ่แบ่งใน `STATE.md` → `## Plan`
5. **Finish** — QA PASS + SR ไม่มี blocker + handoff ครบ

### Handoff & สลับ editor

ทุก agent จบด้วย **`## Handoff summary`** → orch เขียน `STATE.md` → `## Last handoff`

สลับ Cursor ↔ Claude ↔ Hermes:

1. จบ turn ปัจจุบัน (handoff ใน STATE)
2. `zsh tools/refresh.sh` หลัง `git pull`
3. แชตใหม่: `Use loom-orch at L1: continue from STATE`

ดู [`docs/handoff.md`](docs/handoff.md)

### Task scope & kind (`STATE.md`)

| ฟิลด์ | ค่า | ใช้เมื่อ |
|-------|-----|----------|
| `scope` | full-stack · api-only · fe-only · motion-heavy · audit-only | เลือก maker |
| `task_kind` | feature · bug · audit-only | เส้นทางบั๊ก vs ฟีเจอร์ |
| `tdd_policy` | logic-only · off · always | เมื่อเขียนเทสก่อน |

**`qa-browser`** + **`local-cdp`** ติดตั้งตอน `refresh.sh` / `init.sh` — [Browser QA](#browser-qa--การตรวจ) · [docs/browser-qa.md](docs/browser-qa.md)

สเปก loop → [LOOP.md](LOOP.md) · Process gates → [docs/loop-process.md](docs/loop-process.md) · สถาปัตยกรรม → [docs/hexagonal-project-structure.md](docs/hexagonal-project-structure.md) · เทส → [docs/test-authoring.md](docs/test-authoring.md) · TypeScript → [docs/typescript-packages.md](docs/typescript-packages.md)

---

## Browser QA & การตรวจ

**FE/UI เท่านั้น** `loom-orch` **ข้าม** browser gate เมื่อ `api-only`, `audit-only` หรือ AC ไม่มี UI

### โหมด (`loop.config.json` → `qa_browser`)

| ค่า | ความหมาย |
|-----|----------|
| `auto` | `local-cdp` ถ้ามี chrome-devtools-mcp (หลัง `refresh.sh`) ไม่งั้น `browser-use` |
| `local-cdp` | localhost ผ่าน MCP — ไม่ต้อง API key |
| `browser-use` | cloud browser — ต้อง `BROWSER_USE_API_KEY` (orch gate ก่อน QA) |

### Runtime gate (มี UI AC ใน loop)
...
| ตัวเลือก | การทำ |
|----------|--------|
| **A** | วาง API key → `~/.loom/browser-use.env` |
| **B** | Agent self-signup (qa skill) |
| **C** | เปลี่ยนเป็น `local-cdp` + ติดตั้ง MCP |

### `loom-qa` รันอะไร

1. **Deterministic** — `npm test`, lint, build
2. **Browser** — เฉพาะ UI/layout/flow
3. **ไม่** PASS จาก self-report โดยไม่รันซ้ำ
...
ดู [docs/typescript-packages.md](docs/typescript-packages.md)
...
| ติดตั้ง | `zsh tools/init.sh` | เปิดโฟลเดอร์ | `zsh tools/init.sh` |
...
```zsh
zsh tools/install-dash-hooks.sh   # รวมใน init.sh — ข้ามแพลตฟอร์มที่ไม่มี
```
...
`init.sh` คัดลอก subagent ไป `~/.claude/agents/` เมื่อ detect Claude ได้ — ใช้ได้ทุกโปรเจกต์ทันที
...
เมื่อ detect Hermes ได้ `init.sh` ติดตั้ง team skills (`loom-start loom-orch loom-pm loom-ux-ui loom-fe loom-motion loom-be loom-full-stack loom-qa LOOM`)
...
### `qa_browser` และ `locale`

| ฟิลด์ | ความหมาย |
| ----- | ------- |
| `qa_browser` | `auto` \| `local-cdp` \| `browser-use` — backend browser สำหรับ FE/UI ([Browser QA](#browser-qa--การตรวจ)) |
| `locale` | `en` \| `th` \| `auto` — ข้อความที่ agent แสดงให้ user |
...
ต้องมี `ponytail-review` / `ponytail-audit` — `init.sh` ติดตั้งให้ — ดู [ขั้น 1](#1-ติดตั้งทีม-ครั้งเดียวต่อเครื่อง--รันจาก-base)
...
เปิดอัตโนมัติตอน `init.sh` · ตอนเริ่ม `loom-orch` จะถาม **「เปิด dashboard ดู agent ทำงานไหม? [Y/n]」** (default Y) แล้วเปิด browser ที่ `http://localhost:19000` — เรียกซ้ำได้ปลอดภัย
...
สร้างสำหรับทีมนี้ (ติดตั้งไป `~/.hermes/skills/` โดย `init.sh`):
...
ติดตั้งไป `~/.agents/skills/` ตอน init (ผ่าน `npx skills add` ถ้ามี):
...
| **perf-lighthouse** | fe, fe-mo | Lighthouse / CWV — FE เท่านั้น |
...
| **qa** → Hermes **`qa-browser`** | qa | cloud browser — [browser-use](https://github.com/browser-use/browser-use); **local-cdp** ผ่าน MCP ตอน `refresh.sh` |
| **test-master** *(optional)* | be, fe, qa, pm | เขียนเทสตามสถานการณ์ — `test-master-gate.sh` ([docs/test-authoring.md](docs/test-authoring.md)) |
...
| **test-master** *(optional)* | be, fe, qa, pm | `test-master-gate.sh` หรือ `install-optional-test-master.sh` |
...
```
README.md · README-TH.md       เริ่มต้นเร็ว (default บน GitHub)
README.full.md · README-TH.full.md   เอกสารเต็ม
docs/
  loop-process.md          process gates (evidence, debug, SR, finish)
  browser-qa.md            โหมด browser FE/UI + gate API key
  test-authoring.md        แยก verify FE/BE · test-master ทางเลือก
  typescript-packages.md   profiles npm แนะนำ (ts-common / ts-be / ts-nest)
  hexagonal-project-structure.md   BE hex + FE clean layers
  handoff.md               สลับ editor / ต่อ session
...
tools/                     only in Base — every control folder shares via ~/.loop-base
  refresh.sh               sync: ~/.loop-base · CLI · agents · hooks · browser QA (อัตโนมัติหลัง pull)
  init.sh                ติดตั้งเต็ม: refresh + external skills + L3 + เปิด dashboard
  sync-agents.sh           sync agent defs ทุกแพลตฟอร์ม (source = .claude/agents/)
  install-browser-qa.sh    chrome-devtools-mcp + qa-browser skill
  qa-browser-gate.sh       resolve qa_browser · gate API key
  test-master-gate.sh      ลง test-master แบบ agent ถาม user
  add-typescript-deps.sh   profiles npm TypeScript ของ Loom
  typescript-deps.json     manifest สำหรับ add-typescript-deps.sh
  install-optional-test-master.sh   ติดตั้ง test-master ตรง (แนะนำใช้ gate)
  ensure-loom-home.sh      secrets ~/.loom + gitignore
  ensure-control-gitignore.sh   gitignore control folder สำหรับ key
...
ถ้าติดตั้ง Loom **ก่อน v1.0.2** (หรือก่อนเปลี่ยนชื่อเป็น `loom-*`) เครื่องอาจยังมี **agent ID เก่า** (`loop-start`, `loop-orch`, `pm`, `be`, `fe-anim`, …) ค้างใน Claude Code, Hermes หรือ **Cursor Settings → Agents → Subagents** คนติดตั้งใหม่ผ่าน `init.sh` ไม่ต้องทำส่วนนี้ — สำหรับ **เครื่องที่ติดตั้งไปแล้ว** เท่านั้น
...
| **Hermes** | `ls ~/.hermes/skills/` | `loom-start`, `loom-orch`, … `loom-qa`, `LOOM` (+ symlink ภายนอกถ้ารัน `init.sh` แล้ว) |
...
# Hermes (ลบทั้งชุด team skills — init จะติดตั้งใหม่)
...
git pull
zsh tools/purge-legacy-agents.sh    # แนะนำก่อน init บนเครื่องเก่า
zsh tools/init.sh
```

`init.sh` รัน `sync-agents.sh` (รวม Cursor subagents) แต่**ไม่ลบ**ชื่อ skill Hermes เก่าเอง — ควรรัน `purge-legacy-agents.sh` ก่อนเมื่ออัปเกรดจาก v1.0.2 ลงมา
